...
//Thư viện dùng để gọi Database của Firebase
...
export default function Centres({ navigation }) {
  ...
  //Dùng để get Data từ một phần: await db.collection("users").doc("iddoc").get();
  React.useEffect(async () => {
    const { docs } = await db.collection("centre").get();
    setData(
      docs.map((doc) => {
        const id = doc.id;
        return { ...doc.data(), id };
            ...
            renderItem={({ item }) => (
              <View style={{ marginBottom: 20 }}>
                ...
                >
                  ...
                  >
                    <Image
                      style={{ width: 54, height: 26, borderRadius: 6 }}
                      source={{
